...
enum class PropertyEffectKind {
    // Non-dataflow control/presentation: the owning element applies the change to
    // its own UI/display state; the session does nothing (no rerun, no cache, no
    // generation) and it is valid in any player state.
    UiControl,
    // The sink must re-derive its display from its cached INPUT buffer (output
    // depends on buffer content + property), so the session reprocesses the sink
    // from cache. No upstream rerun.
    SinkDisplay,
    MetadataOutput,
    PayloadOutput,
    CapsOutput,
...
struct PropertyEffect {
    PropertyEffectKind kind = PropertyEffectKind::UiControl;
