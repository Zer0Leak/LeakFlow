...
// not resumable, but the cache is kept so edits can recompute if replay-safe, or
// be rejected if replay would cross a non-replayable element. Paused vs Idle differ
// by whether there is more to stream.
...
public:
    ...
    // Runtime telemetry controls whether framework telemetry primitives update
    // and whether threaded boundaries publish live telemetry snapshots. Graph
    // defaults this on; headless CLI can keep it off to avoid overhead.
