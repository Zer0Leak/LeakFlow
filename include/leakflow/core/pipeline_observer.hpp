...
struct PipelineTelemetrySnapshot {
    ...
    // Size = monitoring counters/gauges; Duration = profiling timing. The graph UI
    // renders these in separate "Telemetry" and "Profile" sections.
...
struct PipelineTopologySnapshot {
    ...
    bool runtime_telemetry_enabled = true;
};
...
struct PipelineTelemetryChangeObservation {
    PipelineEndpointSnapshot element;
