...
int main()
{
    ...
    bool saw_queue_telemetry_event = false;
    for (const auto& event : observer->events) {
        if (event.kind == leakflow::PipelineEventKind::TelemetryChanged && event.telemetry_change &&
            event.telemetry_change->telemetry_name == "received" &&
            (event.telemetry_change->element.element_name == "qa" ||
             event.telemetry_change->element.element_name == "qb")) {
            saw_queue_telemetry_event = true;
            break;
        }
    }
    if (!expect(saw_queue_telemetry_event, "threaded Queue did not publish telemetry changes")) {
        return 1;
    }
    ...
    sink_element->clocks.clear();
    std::stop_source disabled_telemetry_source;
    (void)pipeline.run_threaded(disabled_telemetry_source.get_token(), {}, nullptr, false);

    bool saw_disabled_telemetry_event = false;
    for (const auto& event : observer->events) {
        if (event.kind == leakflow::PipelineEventKind::TelemetryChanged) {
            saw_disabled_telemetry_event = true;
            break;
        }
    }
    if (!expect(!saw_disabled_telemetry_event, "threaded run published telemetry while telemetry was disabled")) {
        return 1;
    }
    if (!expect(sink_element->clocks.size() == 10, "disabled-telemetry run must still deliver every buffer")) {
        return 1;
    }
