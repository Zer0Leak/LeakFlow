...
int main()
{
    ...
    if (!expect(descriptors[0].elements[6].type_name == "PearsonPoiFinder",
            "PearsonPoiFinder descriptor type name was wrong")) {
        return 1;
    }
    if (!expect(descriptors[0].elements[6].output_pads[0].caps().type() == crypto_plugin::correlation_poi_caps_type,
            "PearsonPoiFinder descriptor output caps were wrong")) {
        return 1;
    }
    if (!expect(descriptors[0].elements[7].type_name == "CorrelationPoiToPlotAnnotations",
