...
{
    return PropertyEffect{PropertyEffectKind::UiOnly, PropertyInvalidationScope::None, {}};
}
...
int main()
{
    ...
    // ui-only change: no rerun, no epoch bump.
    {
        std::shared_ptr<NumberSource> src;
        std::shared_ptr<PassThrough> mid;
        std::shared_ptr<CapturingSink> sink;
        ...
        if (!expect(session.generation() == 1, "ui-only change bumped the epoch")) {
            return 1;
        }
        if (!expect(sink->process_count == before, "ui-only change triggered a rerun")) {
            return 1;
        }
    }
