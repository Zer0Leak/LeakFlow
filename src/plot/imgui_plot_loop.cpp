...
bool draw_vertical_slider(std::string_view tooltip_label, const std::string &imgui_id, int &value, int max_value,
    ...
    if (ImGui::IsItemHovered()) {
        const auto tooltip =
            std::string(tooltip_label) + "\ntrace " + std::to_string(value) + " / " + std::to_string(max_value);
...
void draw_annotation_marker(ImDrawList &draw_list, ImVec2 marker, float marker_radius,
    ...
    if (colors.size() == 1 || colors_are_uniform(colors)) {
        draw_list.AddCircleFilled(marker, marker_radius, ImGui::GetColorU32(colors.front()), 12);
        return;
    }
...
void plot_snapshot_annotations(const TracePlotSnapshot &snapshot, const float *values,
    ...
    for (const auto &group : groups) {
        if (group.size() > 1 && annotation_group_shares_marker(group)) {
            ...
            draw_annotation_marker(*draw_list, marker, marker_radius, colors);
            draw_list->AddCircle(marker, marker_radius + 1.0F, ImGui::GetColorU32(ImVec4(0.0F, 0.0F, 0.0F, 0.75F)), 12,
                                 1.0F);
        ...
        for (std::size_t index = 0; index < group.size(); ++index) {
            ...
            draw_annotation_marker(*draw_list, marker, marker_radius, {color});
            draw_list->AddCircle(marker, marker_radius + 1.0F, ImGui::GetColorU32(ImVec4(0.0F, 0.0F, 0.0F, 0.75F)), 12,
                                 1.0F);
...
void plot_snapshot(PlotRuntime &runtime, const TracePlotSnapshot &snapshot, const TracePlotSnapshot &axis_source) {
    ...
    plot_snapshot_annotations(snapshot, values, x_values, annotation_styles);
}
