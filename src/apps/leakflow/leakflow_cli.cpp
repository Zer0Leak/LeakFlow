...
linked_element_factory_registry(std::shared_ptr<leakflow::plot::PlotRuntime> plot_runtime) {
    ...
    // own the concrete payloads and injected into the extras buffer-file factories
    // (extras is the layer that has both Torch and the HDF5 storage backend).
    ...
    ml_plugin::register_element_factories(registry);
...
public:
    ...
    int run(std::string_view expression, RunExpressionOptions options) {
        ...
        const auto normalized_expression = normalize_line_continuations(expression);
        parse_expression(normalized_expression);
        const bool telemetry_enabled = options.telemetry_enabled.value_or(options.graph);
        configure_profiling(options);

        if (options.graph) {
            return run_with_graph(options.auto_start, telemetry_enabled);
        }
        if (has_plot_elements()) {
            return run_with_live_plot(telemetry_enabled);
        }
        ...
        session.set_telemetry_enabled(telemetry_enabled);
        setup_profiling(session.pipeline());
        auto result = session.run_once();
        finish_profiling(session.pipeline());
...
private:
    int run_with_live_plot(bool telemetry_enabled) {
        // Non-graph plotting should still be live: open the TracePlot window while
        // the pipeline runs, then keep it open after natural EOS for inspection.
        // Closing the window, Ctrl+C, or a worker failure requests cooperative stop.
        leakflow::PipelineSession session(std::move(pipeline_));
        session.set_telemetry_enabled(telemetry_enabled);
        setup_profiling(session.pipeline());
        InterruptScope interrupt;
        ...
        std::jthread worker([&](std::stop_token token) {
            ...
            const auto stop_run = [&]() {
                ...
                session.request_resume(); // wake a paused/safe-pointed run
            };
            std::stop_callback window_cb(token, stop_run);
            std::stop_callback interrupt_cb(interrupt.token(), stop_run);
        ...
        leakflow::plot::PlotLoopOptions plot_options;
        plot_options.window_title = "LeakFlow TracePlot";
        plot_options.show_controls_window = false;
        plot_options.should_close = [&]() {
            return worker_failed.load() || interrupt.token().stop_requested();
        };
        ...
        try {
            leakflow::log::write(log::LogRecord{
                .level = log::LogLevel::Debug,
                .component = "cli",
                .message = "entering live plot runtime loop",
            });
            leakflow::plot::run_until_closed(*plot_runtime_, plot_options);
            stop_worker();
        } catch (...) {
            stop_worker();
            throw;
        ...
        leakflow::log::write(log::LogRecord{
            .level = log::LogLevel::Debug,
            .component = "cli",
            .message = "finished live plot pipeline expression",
        });
        return 0;
    }

    int run_with_graph(bool auto_start, bool telemetry_enabled) {
    ...
    [[nodiscard]] bool has_plot_elements() const {
        return std::any_of(elements_.begin(), elements_.end(), [](const std::shared_ptr<Element> &element) {
            if (!element) {
                return false;
            }
            return element->element_type() == "TracePlot" || element->element_kclass().starts_with("Sink/Plot");
        });
    }
    ...
    // Profiling is a frontend concern: the runtime gate lives on the pipeline, the
    // table and trace file are produced here. See docs/design/profiling.md.
    void configure_profiling(const RunExpressionOptions &options) {
        print_profile_ = options.print_profile;
        profile_file_ = options.profile_file;
        profiling_enabled_ = options.print_profile || options.profile_file.has_value();
        if (profile_file_) {
            trace_sink_ = std::make_unique<leakflow::TelemetryTraceSink>();
        }
    }

    void setup_profiling(Pipeline &pipeline) {
        if (!profiling_enabled_) {
            return;
        }
        pipeline.set_profiling_enabled(true);
        if (trace_sink_) {
            pipeline.set_trace_sink(trace_sink_.get());
        }
    }

    void finish_profiling(const Pipeline &pipeline) {
        if (print_profile_) {
            print_profile_table(pipeline);
        }
        if (profile_file_ && trace_sink_) {
            std::ofstream file(*profile_file_, std::ios::trunc);
            if (!file) {
                leakflow::log::write(log::LogRecord{
                    .level = log::LogLevel::Error,
                    .component = "cli",
                    .message = "could not open profile file: " + *profile_file_,
                });
            } else {
                file << trace_sink_->to_chrome_json();
            }
        }
    }

    void print_profile_table(const Pipeline &pipeline) {
        struct Row {
            std::string element;
            leakflow::TelemetryDurationReport report;
        };
        std::vector<Row> rows;
        for (const auto &element : pipeline.elements()) {
            for (auto &report : element->duration_reports()) {
                rows.push_back(Row{element->name(), std::move(report)});
            }
        }
        // Bottleneck first: the whole point of the table is the largest total.
        std::sort(rows.begin(), rows.end(),
                  [](const Row &lhs, const Row &rhs) { return lhs.report.total_ns > rhs.report.total_ns; });

        const auto ms = [](std::uint64_t ns) {
            std::ostringstream out;
            out << std::fixed << std::setprecision(3) << static_cast<double>(ns) / 1'000'000.0;
            return out.str();
        };
        const auto us = [](std::uint64_t ns) {
            std::ostringstream out;
            out << std::fixed << std::setprecision(1) << static_cast<double>(ns) / 1'000.0;
            return out.str();
        };

        output_ << "\nLeakFlow profile (wall time)\n";
        output_ << std::left << std::setw(20) << "ELEMENT" << std::setw(18) << "OP" << std::right << std::setw(8)
                << "CALLS" << std::setw(12) << "TOTAL ms" << std::setw(12) << "AVG us" << std::setw(12) << "MAX us"
                << '\n';
        if (rows.empty()) {
            output_ << "(no timing recorded)\n";
            return;
        }
        for (const auto &row : rows) {
            output_ << std::left << std::setw(20) << row.element << std::setw(18) << row.report.name << std::right
                    << std::setw(8) << row.report.count << std::setw(12) << ms(row.report.total_ns) << std::setw(12)
                    << us(row.report.average_ns()) << std::setw(12) << us(row.report.max_ns) << '\n';
        }
    }
    ...
    std::unique_ptr<leakflow::TelemetryTraceSink> trace_sink_;
    bool profiling_enabled_ = false;
    bool print_profile_ = false;
    std::optional<std::string> profile_file_;
