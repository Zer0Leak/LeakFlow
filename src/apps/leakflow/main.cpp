...
void print_help() {
    ...
    std::cout << "       leakflow [OPTIONS] run [--graph] [--auto-start] "
                 "[--telemetry|--no-telemetry] EXPRESSION\n";
    std::cout << "       leakflow [OPTIONS] --pipeline NAME [--set "
                 "ELEMENT.PROPERTY=VALUE]\n";
    std::cout << '\n';
    std::cout << "Logging, summary, and runtime options:\n";
    ...
    std::cout << "  --telemetry             enable runtime telemetry sampling for run\n";
    std::cout << "  --no-telemetry          disable runtime telemetry sampling for run\n";
    std::cout << "                          (default: off headless, on with --graph)\n";
    std::cout << "  --print-profile         print the per-element timing table at exit\n";
    std::cout << "  --profile-file PATH     write a Chrome trace (chrome://tracing / Perfetto)\n";
    std::cout << "                          (both enable timing; see docs/design/profiling.md)\n";
    std::cout << '\n';
    ...
    try {
        ...
        if (first_argument == "run") {
            ...
            while (expression_index < argc) {
                ...
                if (run_option == "--telemetry") {
                    telemetry_enabled = true;
                    ++expression_index;
                    continue;
                }
                if (run_option == "--no-telemetry") {
                    telemetry_enabled = false;
                    ++expression_index;
                    continue;
                }
                if (run_option == "--print-profile") {
                    print_profile = true;
                    ++expression_index;
                    continue;
                }
                if (run_option == "--profile-file") {
                    if (expression_index + 1 >= argc) {
                        std::cerr << "--profile-file requires a path\n";
                        return 1;
                    }
                    profile_file = argv[++expression_index];
                    ++expression_index;
                    continue;
                }
                break;
            ...
            return leakflow::cli::run_expression(
                ...
                std::cout,
                leakflow::cli::RunExpressionOptions{
                    .graph = graph,
                    .auto_start = auto_start,
                    .telemetry_enabled = telemetry_enabled,
                    .print_profile = print_profile,
                    .profile_file = std::move(profile_file),
                });
        }
