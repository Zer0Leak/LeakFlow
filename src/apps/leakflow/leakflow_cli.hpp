...
struct RunExpressionOptions {
    ...
    // Nullopt means frontend default: off for headless, on for --graph.
    std::optional<bool> telemetry_enabled;
    // Profiling (timing) output. Either enables the profiling gate; profile_file
    // additionally records per-event spans for a Chrome trace. See
    // docs/design/profiling.md.
    bool print_profile = false;          // --print-profile: print the timing table at exit
    std::optional<std::string> profile_file; // --profile-file PATH: write Chrome trace JSON
};
