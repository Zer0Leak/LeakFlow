...
{
    if (spec.name.empty()) {
        throw std::invalid_argument("telemetry name cannot be empty");
    }
    if (property_value_is_null(spec.value_type)) {
        throw std::invalid_argument("telemetry value type cannot be null");
    }
}

TelemetrySpec make_duration_telemetry_spec(std::string name, std::string description, std::string unit)
{
    TelemetrySpec spec(std::move(name), std::int64_t{0}, std::move(description), std::move(unit));
    spec.kind = TelemetryKind::Duration;
    return spec;
}
...
ElementDescriptor with_common_element_properties(ElementDescriptor descriptor)
{
    ...
    // Every element advertises a built-in "process" timing channel: the executor
    // wraps each process_pads() call in it when profiling is enabled. Declaring it
    // here makes it visible to leakflow-ls (like the common "name" property) and
    // pre-creates the matching stat through configure_from_descriptor.
    const auto has_process_telemetry = std::any_of(descriptor.telemetry_specs.begin(),
        descriptor.telemetry_specs.end(),
        [](const TelemetrySpec& spec) { return spec.name == "process"; });
    if (!has_process_telemetry) {
        descriptor.telemetry_specs.insert(descriptor.telemetry_specs.begin(),
            make_duration_telemetry_spec("process", "wall time spent in this element's process step"));
    }
