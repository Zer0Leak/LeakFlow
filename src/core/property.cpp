...
{
    switch (kind) {
    case PropertyEffectKind::UiOnly:
        return "ui-only";
...
void validate_property_effect(const PropertyEffect& effect)
{
    ...
    if (effect.kind == PropertyEffectKind::UiOnly && effect.scope != PropertyInvalidationScope::None
        && effect.scope != PropertyInvalidationScope::ElementUi) {
        throw std::invalid_argument("ui-only property effects cannot invalidate pipeline dataflow");
    }
