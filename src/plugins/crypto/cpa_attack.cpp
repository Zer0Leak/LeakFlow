...
std::optional<Buffer> CpaAttack::process_inputs(ElementInputs inputs)
{
    ...
    const auto prepared = prepare_inputs(features_payload->tensor(), hypotheses_payload->tensor(), compute_dtype);
    ...
    auto score_result = score_correlation(correlation, score_method_for(score_method_text), selected_channels);
