...
ElementDescriptor AttackStats::descriptor()
{
    return {
        ...
        .metadata_set_by_element = {
            ...
            make_element_metadata_descriptor(
                "attack.stats.success_count", std::int64_t{}, "number of units recovered at rank 1", {"16"}),
            make_element_metadata_descriptor(
...
std::optional<Buffer> AttackStats::process_inputs(ElementInputs inputs)
{
    ...
    auto truth_values = truth_values_for_units(truth_payload->tensor(), attack_payload->unit_indexes(), unit_count);
    ...
    for (std::int64_t unit = 0; unit < unit_count; ++unit) {
        ...
        const auto truth_guess_index = find_guess_index(guess_values, truth_values[static_cast<std::size_t>(unit)]);
        ...
        true_guess_values.push_back(truth_values[static_cast<std::size_t>(unit)]);
