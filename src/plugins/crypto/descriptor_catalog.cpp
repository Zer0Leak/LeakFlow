...
[[nodiscard]] torch::Tensor units_to_tensor(const std::vector<std::int64_t>& units)
{
    auto tensor = torch::empty({static_cast<std::int64_t>(units.size())}, torch::kInt64);
    auto accessor = tensor.accessor<std::int64_t, 1>();
    for (std::size_t index = 0; index < units.size(); ++index) {
        accessor[static_cast<std::int64_t>(index)] = units[index];
    }
    return tensor;
}

[[nodiscard]] std::vector<std::int64_t> units_from_tensor(const torch::Tensor& tensor)
{
    ...
    units.reserve(static_cast<std::size_t>(contiguous.size(0)));
    for (std::int64_t index = 0; index < contiguous.size(0); ++index) {
        units.push_back(accessor[index]);
    }
    return units;
}
...
{
    // CorrelationPayload: [grouped, units, channel_count, feature_count,
        ...
        PayloadCodec{
            .save =
                [](const Payload& payload, leakflow::base::BufferArchiveWriter& archive) {
                    ...
                    archive.write_tensor("units", units_to_tensor(correlation->units()));
            ...
            .load =
                [](const leakflow::base::BufferArchiveReader& archive) -> std::shared_ptr<Payload> {
                    return std::make_shared<CorrelationPayload>(
                        ...
                        units_from_tensor(archive.read_tensor("units")),
    ...
    // CorrelationPoiPayload: [units, list<result tensor>, score_name].
    codecs.register_codec(
