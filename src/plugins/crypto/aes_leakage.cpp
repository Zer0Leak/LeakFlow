      ...
      .property_specs =
          {
              PropertySpec(
                  "units", Units::none(),
                  "AES state byte indexes to model, e.g. [0] / [0:16] / [0,2:4]; none/[] = all bytes",
                  "", std::monostate{}, "",
                  PropertyEffect{
...
std::optional<Buffer> AesLeakage::process_inputs(ElementInputs inputs) {
  ...
  {
    // Opt-in internal op scope: times just the leakage tensor computation, nested
    // under this element's process timer. See docs/design/profiling.md.
    auto scope = profile_scope("leakage_compute");
