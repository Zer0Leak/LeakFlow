...
The **Queue is passive** — it runs no thread; T1 pushes, T2 pulls.
...
✔ Validates: segment threading (threads = components cut at Queues), push/pull with
a passive Queue, backpressure/drop policy, cooperative stop via stop token +
poll-timeout (= the `NoData` mechanism).
...
| Queue = rate + thread + generation boundary; passive push/pull | 5, 6, 7 |
