...
Effective behaviour of a property change:

```
effective scope = stronger of (
    the changed property's declared PropertyEffect/scope,
    escalation forced by any non-replayable element in the replay-set )
```
...
```
full restart is forced  ⇔  the replay-set contains an element with can_replay()==false
otherwise               ⇒  partial rerun_from(X)
```

A non-replayable element **upstream** of `X` never forces escalation (it is never
in the replay-set; its cached output is reused). It forces a full restart only
when it is `X` itself or reachable downstream of `X`.

Full restart = `stop_all(); start_all(); run_sweep();` (a clean reset, relying on
the D3 lifecycle-reset clause), and bumps the epoch.
