...
`leakflow run` opens the ImGui/ImPlot loop immediately when the expression
contains a `TracePlot`. The pipeline runs on a worker thread, so live sources can
update the plot while buffers arrive. After natural end-of-stream, the plot
window stays open for inspection until you close it. Closing the window or
pressing Ctrl+C requests a cooperative stop from the pipeline worker.
...
The controls window includes a `Focus window:` row: one button per open plot
window (traces, scores, table, graph) that brings it to the front on click — a
reliable alternative to `Ctrl+Tab`, which can be unreliable depending on the
window manager's keyboard focus / modifier handling.
