...
- `leakflow_ml` exists and provides generic Torch numeric APIs for Gaussian
  ...
  fragmentation, rectangular-alignment, and combined evaluation contract. Its
  schema-v6 post-A4 correction defines semantic partition
  separation and corrected semantic partition quality, default-active with
  power semantics and normalized inactive with semantics off.
...
contingency Heatmap tab. A separate post-A4 schema-v6 correction adds semantic
partition separation and the power-mode-default corrected quality used for
comparisons across predicted-cluster counts. Zarr parity remains a separate
deferred candidate.
...
- Phase A4 added `ClusteringEvaluationPayload` and `ClusteringEvaluate`. The
  element aligns
...
- The post-A4 quality correction advances the numeric result schema to v6 and
  defines `semantic_partition_separation = 1 - D_within / D_all` at metric ID
  27.
  ...
  collapse and all-singleton fragmentation score `0`. It defaults active with
  power semantics, normalizes inactive with semantics off, and supports an
  explicit false opt-out. It is metric ID 28 in the 29-descriptor schema.
...
- `ClusteringMetricsTablePlot`: consumes that result on `sink`. The original A4
  ...
  separation and semantic partition quality. Exact and Combined now use
  Overview-style rows with the same shape/count context and selected parameters:
  Exact adds all ten exact metrics, while Combined adds only preferred quality,
  separation, and pair recall. Both append comparison rows directly in
  accumulate mode and keep metric-specific support in hover. Semantic, Fragmentation, and
  Alignment retain long-form detail, while Parameters
