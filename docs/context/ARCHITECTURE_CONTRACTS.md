...
extends the numeric result to schema v6 with a final 29-descriptor inventory and
the `no_semantic_variation` undefined reason.
...
- `leakflow_ml` owns only the generic numeric options, algorithms, and
  ...
  rectangular alignment records are implemented there. Schema v6 also owns
  semantic partition separation and the power-mode-default corrected semantic
  partition quality; both remain generic and GMM-independent. It remains Torch-numeric
...
- `leakflow_plugins_ml` owns `ClusteringEvaluate` and the core-derived
  ...
  `Buffer`, not duplicated in the payload. The schema version is `6`, with
  metric IDs `0..28`. Versioned persistence is unblocked but deferred.
...
For semantic mode, schema v6 defines total semantic pair cost `D_all` over all
...
with `no_semantic_variation`. `semantic_partition_quality` is the
harmonic mean of that separation and exact pair recall. This second term makes
all-singleton fragmentation score zero; a perfect partition scores one. It
defaults active with power semantics, normalizes inactive with semantics off,
and supports explicit false opt-out. Semantic partition separation is metric ID
27 and semantic partition quality is metric ID 28.
...
downstream invalidation. `semantic_partition_quality` defaults true, is active
with power semantic evaluation, normalizes false with semantics off, and can be
explicitly disabled.
`ClusteringMetricsTablePlot.update_mode` is
...
Overview promotes `semantic_partition_separation` and active
`semantic_partition_quality` as the preferred semantic comparison columns.
Exact and Combined are Overview-style one-row-per-run/unit tables sharing the
...
Both append directly in accumulate mode with no history scrubber, and keep each
metric's denominator/evidence support in hover rather than a visible column.
Plotting does not compute evaluation scores.
