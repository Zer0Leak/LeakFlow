...
- applies commands only at safe points (between units of work — between sweeps
  ...
  live elements;
- uses `PropertyEffect` to choose UI-only / sink-display / metadata-output /
...
- `ui-only` — redraw UI only, no new buffer.
- `sink-display` — update sink/display state only, no upstream rerun.
...
- Plotting: `leakflow_plugins_plot`.
